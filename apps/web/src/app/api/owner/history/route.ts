...
import { listPolicyRequests, readPolicyRequest, requestToApi } from "@/lib/apiServer";
...
export async function GET(request: Request) {
  try {
    ...
    const requests = (await Promise.all(policies.map(async (policy) => {
      ...
      return rows.map((row) => ({ policy, agent: state.authorized_agent, ...requestToApi(row, Number(state.token_decimals ?? 6)) }));
