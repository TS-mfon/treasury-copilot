// SPDX-License-Identifier: MIT
pragma solidity ^0.8.24;

import {Treasury} from "./Treasury.sol";

library Clones {
    function clone(address implementation) internal returns (address instance) {
        assembly {
            let ptr := mload(0x40)
            mstore(ptr, 0x3d602d80600a3d3981f3363d3d373d3d3d363d73000000000000000000000000)
            mstore(add(ptr, 0x14), shl(0x60, implementation))
            mstore(add(ptr, 0x28), 0x5af43d82803e903d91602b57fd5bf30000000000000000000000000000000000)
            instance := create(0, ptr, 0x37)
        }
        require(instance != address(0), "clone failed");
    }
}

contract TreasuryFactory {
    address public immutable treasuryImplementation;

    event TreasuryCreated(address indexed owner, address indexed treasury, address relayer, address token);

    constructor(address _implementation) {
        require(_implementation != address(0), "implementation zero");
        treasuryImplementation = _implementation;
    }

    function createTreasury(address relayer, address token) external returns (address clone) {
        clone = Clones.clone(treasuryImplementation);
        Treasury(payable(clone)).initialize(msg.sender, relayer, token);
        emit TreasuryCreated(msg.sender, clone, relayer, token);
    }
}
